...
// Enable BGP router ID IP pool mode
...
f, watchersReady := newFixture(t, ctx, require.New(t), daemonCfg)
...
nodes := make([]*v2.CiliumNode, numNodes)
for i := 0; i < numNodes; i++ {
nodes[i] = &v2.CiliumNode{
ObjectMeta: meta_v1.ObjectMeta{
...
// Create BGP cluster config with multiple instances
instances := make([]v2.CiliumBGPInstance, numInstances)
for i := 0; i < numInstances; i++ {
instances[i] = v2.CiliumBGPInstance{
Name:     string(uuid.NewUUID()),
LocalASN: ptr.To[int64](65000 + int64(i)),
}
}
...
BGPInstances: instances,
},
}

// Insert cluster config first
_, err := f.bgpcClient.Create(ctx, clusterConfig, meta_v1.CreateOptions{})
req.NoError(err)

// Wait for initial reconciliation
time.Sleep(100 * time.Millisecond)

// Concurrently insert all nodes - this triggers concurrent router ID allocation
errChan := make(chan error, numNodes)
for _, node := range nodes {
go func(n *v2.CiliumNode) {
_, err := f.nodeClient.Create(ctx, n, meta_v1.CreateOptions{})
errChan <- err
}(node)
}

// Wait for all node creations
for i := 0; i < numNodes; i++ {
req.NoError(<-errChan)
}
...
// Should have node configs for all nodes
assert.Equal(c, numNodes, len(nodeConfigs.Items))

// Collect all allocated router IDs across all instances
allocatedIDs := make(map[string]bool)
for _, nc := range nodeConfigs.Items {
assert.Equal(c, numInstances, len(nc.Spec.BGPInstances))
for _, instance := range nc.Spec.BGPInstances {
if instance.RouterID != nil {
// Check for duplicate router IDs
if allocatedIDs[*instance.RouterID] {
assert.Fail(c, "Duplicate router ID allocated", "RouterID: %s", *instance.RouterID)
}
allocatedIDs[*instance.RouterID] = true
}
}
}

// Should have unique router IDs for all node-instance pairs
expectedTotal := numNodes * numInstances
assert.Equal(c, expectedTotal, len(allocatedIDs), "Should have %d unique router IDs", expectedTotal)
}, TestTimeout, 50*time.Millisecond)
}
