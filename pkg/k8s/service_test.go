...
func (s *K8sSuite) TestParseService(c *check.C) {
	...
	ipv4ZeroAddrCluster, _ := cmtypes.AddrClusterFromIP(net.ParseIP("0.0.0.0"))
	ipv4InternalAddrCluster, _ := cmtypes.AddrClusterFromIP(fakeDatapath.IPv4InternalAddress)
	ipv4NodePortAddrCluster, _ := cmtypes.AddrClusterFromIP(fakeDatapath.IPv4NodePortAddress)
