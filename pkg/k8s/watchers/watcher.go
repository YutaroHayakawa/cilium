...
func (k *K8sWatcher) delK8sSVCs(svc k8s.ServiceID, svcInfo *k8s.Service, se *k8s.Endpoints) error {
	...
	for portName, svcPort := range svcInfo.Ports {
		...
		for _, feIP := range svcInfo.FrontendIPs {
			addrCluster, ok := cmtypes.AddrClusterFromIP(feIP)
			if !ok {
				return fmt.Errorf("invalid frontend address")
			}
			fe := loadbalancer.NewL3n4Addr(svcPort.Protocol, addrCluster, svcPort.Port, loadbalancer.ScopeExternal)
		...
		for _, k8sExternalIP := range svcInfo.K8sExternalIPs {
			addrCluster, ok := cmtypes.AddrClusterFromIP(k8sExternalIP)
			if !ok {
				return fmt.Errorf("invalid k8s external IP address")
			}
...
func genCartesianProduct(
	...
	for fePortName, fePort := range ports {
		...
		addrCluster, _ := cmtypes.AddrClusterFromIP(fe)
