...
func (m *LBMockMap) UpsertService(p *datapathTypes.UpsertServiceParams) error {
	...
	if !found {
		addrCluster, _ := cmtypes.AddrClusterFromIP(p.IP)
		frontend := lb.NewL3n4AddrID(lb.NONE, addrCluster, p.Port, p.Scope, lb.ID(p.ID))
...
func (m *LBMockMap) AddBackend(b *lb.Backend, ipv6 bool) error {
	...
	be := lb.NewBackendWithState(id, b.Protocol, b.AddrCluster, port, b.State, false)
	m.BackendByID[id] = be
